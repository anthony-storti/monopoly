...
board = Board(tiles=list(), current_player=0, players=list(), cards=list(), pieces=['images/car.png',
              'images/hat.png', 'images/dog.png', 'images/thimble.png', 'images/wheel.png', 'images/cat.png',
              'images/ship.png', 'images/boot.png', 'images/ship.png'], trade=False)
...
with open('monopoly_squares.csv') as csv_data_file:
    ...
    for row in csv_reader:
        if row[1] == "Property":
            ...
            tile = CardTile(name=row[2], purchasable=False, action=row[2], color=row[5], x=int(row[15]), y=int(row[16]), cost=0, rent=0, rent_1=0,
                            rent_2=0, rent_3=0, rent_4=0, rent_5=0, house_count=0, hotel_count=0)
            board.tiles.append(tile)
        elif row[1] == "Go":
